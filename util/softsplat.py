...
def FunctionSoftsplat(tenInput, tenFlow, tenMetric, strType):
    ...
    assert(strType in ['summation', 'average', 'linear', 'softmax', 'SymmetricSoftMax'])
    ...
    elif strType == 'softmax' or strType == 'SymmetricSoftMax':
        tenInput = torch.cat([ tenInput * tenMetric.exp(), tenMetric.exp() ], 1)


    tenOutput = _FunctionSoftsplat.apply(tenInput, tenFlow)
    #_, _, h, w = tenInput.shape
    #tenFlow[:,0,:,:] = 2 * ((tenFlow[:,0,:,:] / w) - 0.5)
    #tenFlow[:,1,:,:] = 2 * ((tenFlow[:,1,:,:] / h) - 0.5)
    #
    #tenOutput = torch.nn.functional.grid_sample(tenInput, tenFlow.permute(0,2,3,1))
    if strType != 'summation' and strType != 'SymmetricSoftMax':
